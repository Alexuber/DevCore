...
jQuery(document).ready(function () {
  ...
  $sliderContainers.each(function () {
    ...
    var bg = $sliderContainer
      .find('.slider__slide--active')
      .children('.slider__slide__content')
      .data('background');
    // Lighten using tinycolor
    var lightenBy = 15;
    var lighten = tinycolor(bg).lighten(lightenBy).toString();
    var lightenDouble = tinycolor(bg)
      .lighten(lightenBy * 2)
      .toString();
    ...
    $sliderContainer.on('click', '.controls button', function () {
      ...
      if ($clicked.hasClass('next')) {
        ...
        var nextBg = jQuery('.slider__slide--active')
          .next()
          .children('.slider__slide__content')
          .data('background');
        // Lighten using tinycolor
        var nextLighten = tinycolor(nextBg).lighten(lightenBy).toString();
        var nextLightenDouble = tinycolor(nextBg)
          .lighten(lightenBy * 2)
          .toString();
        // Apply lightened background color to container
        ...
        var prevBg = jQuery('.slider__slide--active')
          .prev()
          .children('.slider__slide__content')
          .data('background');
        // Lighten using tinycolor
        var prevLighten = tinycolor(prevBg).lighten(lightenBy).toString();
        var prevLightenDouble = tinycolor(prevBg)
          .lighten(lightenBy * 2)
          .toString();
        // Apply lightened background color to container
